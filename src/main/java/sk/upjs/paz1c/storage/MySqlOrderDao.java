...
public class MySqlOrderDao implements OrderDao {
	...
	@Override
	public List<Order> getByDay(LocalDateTime day) {
		List<Order> orders = getAll();
	...
	public Order save(Order order) throws EntityNotFoundException {
		if (order.getId() == null) { // insert
			...
			Order newOrder = new Order(insert.executeAndReturnKey(values).longValue(), order.getDay(),
					order.isPrepared(), order.getPortions());
	...
	public void insertFoods(Order order) {
		...
		if (order.getPortions() != null && order.getPortions().keySet().size() > 0) {
			...
			for (int i = 0; i < foods.size(); i++) {
				sb.append("(" + order.getId() + ", " + foods.get(i).getId() + ", "
						+ order.getPortions().get(foods.get(i)) + "),");
			}
